...
class TestRandomizer(MetaTestCaseMixin, TestCase):
    ...
    def test_error_condition1(self):
        ...
        self.assertEqual(cm.exception.code, "meta_rando.randomizationlist")
    ...
    @override_settings(SITE_ID=10, RANDOMIZTION_LIST_PATH="/tmp/erik.csv")
    def test_invalid_path(self):
        message = RandomizationListVerifier().message
        self.assertIn("Randomization list has not been loaded.", message)

    @override_settings(
        SITE_ID=10, RANDOMIZATION_LIST_PATH=os.path.join(mkdtemp(), "randolist.csv")
    )
